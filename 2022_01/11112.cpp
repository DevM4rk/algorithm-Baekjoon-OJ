...
unordered_set<string> impossibleSet;        // 테케 반복 할 때 불가능한 맵 저장해서 시간초과 방지
...
int main(){
    ...
    while(t--){
        ...
        while(!pq.empty()){
            ...
            // 불가능한 케이스가 된 경우, 모두 맞춘 경우 끝냄
            if(cnode.g > 31 || impossibleSet.count(cnode.board) || visitMap.count(TARGET)) break;
            ...
            for(int i=0; i<4; i++){
                ...
                int f=0,g=0,h=0;
                for(int j=0; j<9; j++)          // 휴리스틱 값(목표값) 구하기
        ...
        if(visitMap.count(TARGET)){
            ...
            for(auto m : visitMap) impossibleSet.insert(m.first); //불가능한 경우의 보드판 모두 저장 
            cout << "impossible" << "\n";
        }
    }
}